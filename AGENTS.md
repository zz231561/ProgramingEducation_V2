...
2. **驗收後交付**：能以 unit/integration/E2E、API 模擬或部署探針客觀驗證的條件，一律由 agent 自動驗收到全綠後勾 Checkbox、commit 並 push；Zeabur 以 GitHub 部署版為準，不得停在只存在本機的待驗收版本。
   只有主觀視覺／教學感受、credential 或無法安全建立的外部條件才請使用者驗收；部署後不通時先說明根因與修復／回溯選項，不自行回溯。
...
4. **專業匯報**：收尾用 2–3 句口語交代成果、影響與驗收；適合時用精簡前後表格，參數僅於必要時列出。失敗項才補根因、影響、解法與驗證。
...
**部署與營運**：見 `docs/deployment.md`。**push 即自動部署**、migration 自動跑；唯獨改環境變數須手動重啟 service；OAuth 測試模式 100 人上限。
