...
- **🎯 Phase 5 教師端 進行中**（批次 ⑦）：5-1a ✅ 班級資料表（`classes`/`class_members` migration `l8a9b0c1d2e3` + `models/classroom.py`）；**下一步 5-1b** 班級 CRUD API（建立/邀請碼/加入/移除）→ 5-1c Dashboard 骨架 → 5-2 行為收集 → DEV-E seeder → 5-5 作業指派
