...
- **🎯 Phase 5 教師端 進行中**（批次 ⑦）：5-1a ✅ 班級表 + 5-1b-1 ✅ student_profiles 表（學生補填校名/系所/學號/姓名，首次登入 gate）；**下一步 5-1b-2** 班級 CRUD API（6 位數字邀請碼）→ 5-1b-3 加入班級+profile API → 5-1c Dashboard/表單 UI → 5-2 行為收集 → DEV-E seeder → 5-5 作業指派
