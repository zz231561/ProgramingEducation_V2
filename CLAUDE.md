...
- **🎯 Phase 5 教師端 進行中**（批次 ⑦）：5-1b 全後端完成 ✅（班級 CRUD + profile + 加入班級 + 名冊）；5-1c-1 教師班級管理頁 `/teacher` ✅ **UI 驗收通過**（建班/邀請碼/名冊/停用 + avatar 教師入口；含修 `/users/me` 路由碰撞 + 身分切換即時更新選單 + 精簡選單）；**下一步 5-1c-2** 學生 profile 表單 + 首次登入 gate → 5-1c-3 右上角顯示身分 → 5-2 行為收集 → DEV-E seeder → 5-5 作業指派
