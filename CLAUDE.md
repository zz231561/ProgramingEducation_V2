...
- **🎯 Phase 6 進行中**（NotebookLM grounded 教學內容建構）
  ...
  - 6-M ⏳ LLM 模型選型 v2（2026-07-06 定案）：任務導向路由——生成 `gpt-5-mini` / 審查 `gpt-5.4` / content `gpt-5.4` / 對話與分析 `gpt-5.4-mini`；批次費用 ≈ $6.6、儲值 $10（key 已在 .env）；6-M1 分組環境變數待實作（第 5 批）
  - 實機批次（6-3a-3 + 6-4a + deferred-ui + K4d 調參）排第 5 批合併執行
...
- **🎯 Phase 6-U 學生端修正**（2026-07-06 session 定案）：U1a 首登誤顯 Workspace 待製作 / U1b 反思 UI 比例 / U1c 反思顯示 gating / U2a QUIZ 美化 / U2b 移除 LEARN 摘要 / U2c 拔 1-3 章範例 / U2d QUIZ tab 題庫優先 / U2e Workspace 程式碼存檔 / U2f 範例程式
- **執行順序 10 批已定案**（2026-07-06，詳見 roadmap 已確認決策）：① U1 bugs → ② U2b/c → ③ k-graph 拆分+K6 → ④ quiz 模組 → ⑤ 6-M1+實機批次 → ⑥ U2f → ⑦ 教師端 → ⑧ U2e+監控 → ⑨ Phase 7 → ⑩ 5-3/5-4；真人驗收改使用者自測；**下一步：第 1 批 U1a**
