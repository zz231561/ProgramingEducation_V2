...
- **執行順序 10 批已定案**（2026-07-06；晚間修訂 U2f→U2g）：①~⑥ ✅（U2g 含全量 promote 完成）→ ⑥' 6-3c ✅（2026-07-06 程式碼+實機批次：知識點驅動 436 MC 覆蓋 61/62 片 + 57 coding + 舊題複審刪 15；6-3d 弱項綜合測驗組 10/25 題亦同日完成）→ ⑦ 教師端 ✅ → ⑧ U2e ✅（2026-07-16）+監控 → ⑨ Phase 7 → ⑩ 5-3/5-4；**下一步：7-2a/b/c 監控程式碼**（伺服器規劃見 docs/server-plan.md）（LEARN/citation 品質問題由使用者操作回饋 → 6-4b）
...
- **🎯 Phase 7 上線實測進行中**（2026-08-04 起步，使用者選定直接進部署）：部署前置檢查已修兩個生產阻斷——lock 缺 python-multipart（容器啟動即崩，已 docker build 實測修復）+ zeabur.json 未傳 6-M 模型變數（生產會 fallback gpt-4o）；**卡點＝硬體**：待租伺服器 A（Zeabur dedicated 4C8G）+ 伺服器 B（Judge0 專用 2C2G，需 SSH root）、Google OAuth Client、web domain（拓撲見 docs/server-plan.md，SOP 見 docs/deployment.md §A/§C）
