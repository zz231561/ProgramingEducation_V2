...
- **🎯 Phase 5 教師端 進行中**（批次 ⑦）：5-1b 全後端完成 ✅——班級表 + student_profiles 表 + 班級 CRUD（`/classes`）+ profile API（`/profile` upsert）+ 加入班級（`/classes/join` 驗碼 idempotent + profile gate 409）+ 教師名冊（`/classes/{id}/members`）；**下一步 5-1c** 教師 Dashboard/班級管理 UI + 學生 profile 表單（首次登入 gate）+ 右上角顯示身分 → 5-2 行為收集 → DEV-E seeder → 5-5 作業指派
