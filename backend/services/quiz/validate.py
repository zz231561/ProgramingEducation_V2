...
D. **考點有意義 (point_meaningful)**：題目測驗的是概念理解、語法規則或程式行為嗎？
   若考的是**操作細節或瑣碎資訊** → false。例如：軟體安裝步驟、介面按鈕/選單位置、
   「左上角/右下角」等畫面資訊、影片中無關概念的枝節。這類題無法評量程式能力。
...
async def validate_question(
    ...
    try:
        response = await client.chat.completions.create(
            ...
            **chat_model_kwargs(
                model=settings.llm_model_validate, temperature=0.2, max_tokens=500
            ),
        )
    ...
    if not parsed.point_meaningful:
        issues.append(f"考點無意義（操作細節/瑣碎資訊）：{parsed.point_reason}")

    passed = (
        parsed.answer_correct
        and parsed.concept_fits
        and parsed.bloom_appropriate
        and parsed.point_meaningful
    )
    if passed:
    ...
    return ValidationReport(
        ...
        point_meaningful=parsed.point_meaningful,
        issues=issues,
    )
