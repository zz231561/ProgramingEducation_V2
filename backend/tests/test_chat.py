...
# === _is_repeat_evidence（2026-08-06 mastery 證據去重）===

def _msg(role: MessageRole, code: str | None = None, exec_result: dict | None = None) -> ChatMessage:
    return ChatMessage(
        session_id=uuid.uuid4(), role=role, content="x",
        code_snapshot=code, execution_result=exec_result,
    )


def test_repeat_evidence_same_code_and_result():
    from services.chat import _is_repeat_evidence

    nzec = {"exit_code": 1, "status_description": "Runtime Error (NZEC)"}
    history = [
        _msg(MessageRole.USER, "int main(){return 1;}", nzec),
        _msg(MessageRole.ASSISTANT),
    ]
    assert _is_repeat_evidence("int main(){return 1;}", dict(nzec), history) is True


def test_repeat_evidence_changed_code_is_new():
    from services.chat import _is_repeat_evidence

    history = [_msg(MessageRole.USER, "int main(){return 1;}", None)]
    assert _is_repeat_evidence("int main(){return 0;}", None, history) is False


def test_repeat_evidence_changed_execution_is_new():
    from services.chat import _is_repeat_evidence

    history = [_msg(MessageRole.USER, "code", {"exit_code": 1})]
    assert _is_repeat_evidence("code", {"exit_code": 0}, history) is False


def test_repeat_evidence_empty_history_is_new():
    from services.chat import _is_repeat_evidence

    assert _is_repeat_evidence("code", None, []) is False
