...
涵蓋：
- 3 個 section function 各自正確解析 LLM JSON
...
- generate_unit_content orchestrator 呼叫 3 個 section
"""
...
async def test_summary_success():
    payload = {
        ...
        "citations": [{"timestamp": "00:08", "text_excerpt": "今天介紹遞迴函式"}],
    }
    with patched_llm(json.dumps(payload)):
        result = await generate_summary(_fake_concept(), _fake_chunks())
    assert isinstance(result, Summary)
    assert len(result.key_points) == 3
    assert "終止條件" in result.key_points[1]
...
async def test_llm_returns_invalid_json_raises_502():
    with patched_llm("這不是 JSON"):
        with pytest.raises(AppError) as exc:
            await generate_summary(_fake_concept(), _fake_chunks())
...
async def test_llm_returns_wrong_schema_raises_502():
    # 缺 needs_more_source / 多 max_length 違反
    bad_payload = {"key_points": ["a", "b", "c", "d", "e", "f", "g", "h", "i"]}  # > 7
...
@pytest.mark.asyncio
async def test_generate_unit_content_calls_all_three():
    """orchestrator 應依序呼叫 3 個 section。"""
    concept_payload = {
    ...
    summary_payload = {
        "needs_more_source": False, "reason": "",
        "key_points": ["a", "b", "c"], "citations": [],
    }
    # 三次 LLM call 依序回 3 種 payload
    responses = [
        _mock_completion(json.dumps(concept_payload)),
        _mock_completion(json.dumps(examples_payload)),
        _mock_completion(json.dumps(summary_payload)),
    ]
    mock_client = AsyncMock()
    ...
    assert result.summary.key_points == ["a", "b", "c"]
