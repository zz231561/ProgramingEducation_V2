...
> 作業指派（5-5，2026-07-07：TronClass 式文件繳交，migration `q3f4a5b6c7d8`）：

```
assignments                                   -- 教師建立、指派整班的作業
├── id (UUID, PK)
├── class_id (FK → classes, ON DELETE CASCADE) ★ index
├── teacher_id (FK → users, ON DELETE CASCADE)
├── title (varchar 200)
├── description (text, default "")            -- 文字內容說明
├── due_at (timestamptz, nullable)            -- 截止日（軟提示，不硬鎖）
├── is_active (boolean, default true)
├── created_at
└── updated_at (onupdate)

assignment_submissions                        -- 學生繳交（每生每作業一份，重繳覆蓋）
├── id (UUID, PK)
├── assignment_id (FK → assignments, ON DELETE CASCADE) ★ index
├── student_id (FK → users, ON DELETE CASCADE) ★ index
├── text (text, default "")                   -- 學生文字說明
├── score (float, nullable)                   -- 教師評分（NULL = 未批改；CHECK >= 0）
├── feedback (text, default "")               -- 教師評語
├── graded_at (timestamptz, nullable)
├── submitted_at
├── updated_at (onupdate)
└── UNIQUE (assignment_id, student_id) ★

attachments                                   -- 多型附件（作業 / 繳交），檔案內容存 bytea
├── id (UUID, PK)
├── owner_type (varchar CHECK: assignment/submission)  -- 多型 owner，無 FK cascade（service 層顯式刪）
├── owner_id (UUID) ★ index (owner_type, owner_id)
├── filename (varchar 255)
├── content_type (varchar 100)
├── size_bytes (int, CHECK 0 <= size <= 10485760)  -- 單檔 ≤ 10MB，與 model MAX_ATTACHMENT_BYTES 一致
├── content (bytea)                           -- Zeabur 容器 fs ephemeral，存 DB 才不遺失
├── uploaded_by (FK → users, ON DELETE SET NULL, nullable)
└── created_at
```

### Workspace 程式碼存檔（U2e，migration `r4a5b6c7d8e9`）

```
code_files                                    -- 單表兩用：草稿（name IS NULL）+ 命名檔案
├── id (UUID, PK)
├── user_id (FK → users, ON DELETE CASCADE)
...
└── ★ partial unique index (user_id) WHERE name IS NULL  -- 草稿每人一份
```
> 每人命名檔案上限 50（service 層把關，`models/code_file.py MAX_FILES_PER_USER`）
